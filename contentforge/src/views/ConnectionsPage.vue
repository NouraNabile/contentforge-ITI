    ...
    if (data?.connections) {
      data.connections.forEach(conn => {
        ...
        if (p) {
          ...
          p.handle    = conn.handle || null
          p.stats     = conn.stats  || []
          p.permissions = conn.permissions || p.permissions
        }
      })
    }
  } catch {
    // API not working — keep all disconnected
  }
})

// ── META OAUTH: Connect Facebook or Instagram ─────────────────────────────────
async function connectMetaOAuth(platform) {
  platform.loading = true
  ...
  try {
    ...
    // 3. Poll for popup close, then refresh connections
    const timer = setInterval(async () => {
      if (popup?.closed) {
        clearInterval(timer)
...
async function refreshConnections() {
  try {
    ...
    if (data?.connections) {
      // Reset all first
      socialPlatforms.value.forEach(p => {
        p.connected = false
        p.handle = null
        p.stats = []
      })
...
// ── Connect / Disconnect ──────────────────────────────────────────────────────
async function togglePlatform(platform) {
  // Meta platforms (Facebook & Instagram) use OAuth
  if (platform.name === 'Facebook' || platform.name === 'Instagram') {
    if (!platform.connected) {
      await connectMetaOAuth(platform)
      return
    }
    // Disconnect Meta platform
    platform.loading = true
    try {
      await api.delete(`/connections/${platform.name.toLowerCase()}`)
      platform.connected = false
      ...
      alert(`Failed to disconnect ${platform.name}`)
    } finally {
      platform.loading = false
    }
    return
  ...
  // Other platforms (manual form)
  if (!platform.connected) {
    openConnectModal(platform)
  } else {
    // Disconnect other platforms
    platform.loading = true
    try {
      await api.delete(`/connections/${platform.name.toLowerCase()}`)
      platform.connected = false
...
// ── Open modal for manual platforms ───────────────────────────────────────────
function openConnectModal(platform) {
  activePlatform.value = platform
  formData.value = {}
  showModal.value = true
...
// ── Submit form for manual platforms ──────────────────────────────────────────
async function submitForm() {
  if (!activePlatform.value) return
  ...
  const payload = {
    platform: activePlatform.value.name,
    handle: formData.value.username || formData.value.pageName || formData.value.profileUrl || formData.value.channelName || null,
    email: formData.value.email || null,
    connected: true,
    stats: stats,
