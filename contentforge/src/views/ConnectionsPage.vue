  ...
  <AppLayout>
    <div class="p-7 max-w-4xl mx-auto">
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <div class="flex items-center gap-1.5">
                  <div class="w-1.5 h-1.5 rounded-full" :class="platform.connected ? 'bg-green-400' : 'bg-slate-600'">
                  </div>
                ...
                <button @click="togglePlatform(platform)"
                  class="px-3 py-1.5 rounded-lg text-xs font-medium transition-all" :class="platform.connected
                    ? 'bg-rose-600/10 text-rose-400 border border-rose-500/20 hover:bg-rose-600/20'
                    : 'bg-blue-600 text-white hover:bg-blue-500'">
                  {{ platform.connected ? t('connections.disconnect') : t('connections.connect') }}
                </button>
              </div>
            </div>
            ...
            <div v-if="platform.connected" class="p-3 rounded-xl theme-card theme-border space-y-2">
              <div class="grid grid-cols-3 gap-2 text-center">
                <div v-for="stat in platform.stats" :key="stat.label">
                  ...
                  <p class="text-[10px] theme-muted">{{ t(stat.labelKey) }}</p>
                </div>
              </div>
            </div>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center"
        style="background: rgba(0,0,0,0.6)" @click.self="closeModal">
        ...
        <div class="rounded-2xl theme-surface theme-border p-6 w-full max-w-md mx-4 flex flex-col gap-5"
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              <div class="w-9 h-9 rounded-xl flex items-center justify-center text-lg" :class="activePlatform?.iconBg">
                {{ activePlatform?.icon }}
              </div>
              <div>
                <p class="font-medium theme-text text-sm">{{ t('connections.modalTitle', { name: activePlatform?.name })
                }}</p>
                <p class="text-[11px] theme-muted">{{ t('connections.modalSubtitle') }}
                </p>
              </div>
            </div>
          ...
          <div class="flex flex-col gap-3 overflow-y-auto" style="max-height: 55vh;">
            <div v-for="field in currentFields" :key="field.key">
              ...
              <input v-model="formData[field.key]" :type="field.type || 'text'" :placeholder="t(field.placeholderKey)"
...
const socialPlatforms = ref([
  {
    name: 'Instagram', icon: '📸', iconBg: 'bg-pink-500/15', connected: true, handle: '@araby.coffee', permissions: ['Read posts', 'Publish', 'Analytics'], stats: [
      { labelKey: 'connections.stat.followers', value: '12.4K' },
      { labelKey: 'connections.stat.posts', value: '234' },
      { labelKey: 'connections.stat.engRate', value: '4.2%' },
    ]
  },
  {
    name: 'Facebook', icon: '📘', iconBg: 'bg-blue-500/15', connected: true, handle: 'Araby Coffee EG', permissions: ['Read', 'Publish', 'Insights'], stats: [
      { labelKey: 'connections.stat.likes', value: '8.7K' },
      { labelKey: 'connections.stat.reach', value: '45K' },
      { labelKey: 'connections.stat.posts', value: '189' },
    ]
  },
...
const platformFields = {
  LinkedIn: [
    { key: 'profileUrl', labelKey: 'connections.field.profileUrl', placeholderKey: 'connections.field.profileUrlPlaceholder', type: 'url' },
    { key: 'email', labelKey: 'connections.field.email', placeholderKey: 'connections.field.emailPlaceholder', type: 'email' },
  ],
  'Twitter/X': [
    { key: 'username', labelKey: 'connections.field.username', placeholderKey: 'connections.field.usernamePlaceholder', type: 'text' },
    { key: 'email', labelKey: 'connections.field.email', placeholderKey: 'connections.field.emailPlaceholder', type: 'email' },
  ],
  TikTok: [
    { key: 'username', labelKey: 'connections.field.username', placeholderKey: 'connections.field.usernamePlaceholder', type: 'text' },
  ...
  YouTube: [
    { key: 'channelName', labelKey: 'connections.field.channelName', placeholderKey: 'connections.field.channelNamePlaceholder', type: 'text' },
    { key: 'email', labelKey: 'connections.field.email', placeholderKey: 'connections.field.emailPlaceholder', type: 'email' },
  ],
}
...
async function togglePlatform(platform) {
  if (platform.comingSoon && !platform.connected) {
    alert(t('connections.comingSoon', { name: platform.name }))
    return
  }
