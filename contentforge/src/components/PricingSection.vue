  ...
  <section id="pricing" class="py-24 bg-forge-900 relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-6xl mx-auto px-6">
      <div class="text-center mb-16">
        <span class="text-xs font-medium text-amber-400 uppercase tracking-widest mb-4 block">{{ t('pricing.eyebrow')
        }}</span>
        ...
        <div class="flex items-center justify-center gap-3 mt-8">
          ...
          <button @click="annual = !annual" class="relative w-12 h-6 rounded-full transition-colors duration-200"
      ...
      <div class="grid md:grid-cols-3 gap-6">
        <div v-for="plan in plans" :key="plan.name"
          class="rounded-2xl p-7 border transition-all duration-300 flex flex-col" :class="plan.featured
          ...
          <p class="text-slate-500 text-sm mb-6">{{ t(`pricing.plans.${plan.key}.tagline`) }}</p>
          ...
          <!-- CTA button -->
          <a href="#get-started"
            class="block text-center py-3 rounded-xl text-sm font-medium mb-8 transition-all duration-200" :class="plan.featured
