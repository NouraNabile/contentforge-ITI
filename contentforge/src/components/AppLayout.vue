    ...
    <header class="theme-glass border-b sticky top-0 z-50 px-5 py-3 flex items-center justify-between"
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="relative">
          ...
          <!-- Notifications panel -->
          <div v-if="showNotifs"
            class="absolute top-11 w-80 rounded-2xl theme-surface theme-shadow border overflow-hidden z-50 animate-slide-in"
            :style="locale === 'ar' ? 'left:0; right:auto' : 'right:0; left:auto'" style="border-color: var(--border)">
            <div class="flex items-center justify-between px-4 py-3 border-b"
              :class="locale === 'ar' ? 'flex-row-reverse' : ''" style="border-color: var(--border)">
            ...
            <div class="max-h-80 overflow-y-auto scrollbar-thin">
              <div v-for="n in notifications" :key="n.id" @click="markRead(n.id)"
                class="flex gap-3 px-4 py-3 cursor-pointer transition-colors border-b last:border-0" :class="[
                  n.read ? 'opacity-50' : 'hover:bg-blue-500/5',
      ...
      <aside
        ...
        <nav class="space-y-0.5 flex-1">
          <RouterLink v-for="item in navItems" :key="item.path" :to="item.path"
            class="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs transition-all group" :class="[$route.path === item.path
