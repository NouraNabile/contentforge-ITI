    ...
    <div class="p-7 max-w-5xl mx-auto">
      ...
      <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex items-center gap-2">
          <button
            @click="saveAll"
            class="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-all shadow-sm active:scale-95"
          >
      ...
      <div class="flex gap-2 mb-8 border-b border-theme pb-1">
        <button
          ...
          class="px-4 py-2 text-sm font-medium transition-all rounded-t-lg -mb-px"
          :class="
            activeTab === tab.key
      ...
      <section v-if="activeTab === 'Brand Identity'" class="space-y-6" aria-label="Brand Identity">
        <div class="grid md:grid-cols-2 gap-5">
          ...
          <div class="theme-surface rounded-2xl theme-border p-6 space-y-5">
            ...
            <div>
              ...
              <div class="flex gap-2">
                <input
                  ...
                  type="color"
                  class="w-10 h-9 rounded-xl theme-border cursor-pointer bg-transparent p-0 border-0"
                />
                <input
                  v-model="newColorHex"
                  @input="syncHexInput"
                  type="text"
                  placeholder="#3B82F6"
                  class="flex-1 px-3 py-2 rounded-xl theme-border theme-input text-xs theme-text focus:outline-none focus:border-blue-500/40"
                />
                <button
                  @click="addColor"
                  class="px-4 py-2 rounded-xl bg-blue-600 text-white text-xs hover:bg-blue-500 font-medium transition-colors"
                >
                  {{ t('common.add') }}
                </button>
              </div>
            ...
            <div>
              ...
              <div
                class="border-2 border-dashed border-theme rounded-xl p-6 text-center hover:border-blue-500/40 hover:bg-blue-500/5 transition-all cursor-pointer"
        ...
        <div class="theme-surface rounded-2xl theme-border p-6 space-y-6">
          <h3 class="font-display font-600 text-base theme-text flex items-center gap-2">
          ...
          <div class="pt-2">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                class="flex items-center gap-2 px-3.5 py-2 rounded-xl border text-sm transition-all active:scale-95 shadow-sm"
          ...
          <!-- 🌟 [تم النقل هنا] قـسـم الـ Brand Guidelines PDF أصبح الآن جزءاً من الـ Brand Voice -->
          <div class="pt-4 border-t border-theme">
              ...
              <!-- Indicator if file is uploaded -->
              <div v-if="uploadedPdfName"
                class="flex items-center gap-3 p-3 rounded-xl bg-teal-500/10 border border-teal-500/20 mb-4">
                <span class="text-teal-400">📑</span>
                <span class="text-sm text-teal-300 flex-1 truncate">{{ uploadedPdfName }}</span>
                <button @click="removePdf" class="text-slate-500 hover:text-rose-400 transition-colors">
              ...
              <!-- Upload box (image_6013bc.png) -->
              <div
                class="border-2 border-dashed border-theme rounded-xl p-8 text-center hover:border-blue-500/40 hover:bg-blue-500/5 transition-all cursor-pointer"
        ...
        <div class="theme-surface rounded-2xl theme-border p-6">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <button @click="openAddModal"
              class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-medium hover:bg-blue-500 transition-all shadow-sm hover:shadow-blue-600/10 active:scale-95 shrink-0 whitespace-nowrap self-start sm:self-auto">
          ...
          <div v-else-if="!topPosts.length"
            class="py-14 flex flex-col items-center gap-3 rounded-xl theme-card theme-border text-center">
            ...
            <div v-for="post in topPosts" :key="post._id"
              ...
              <div>
                <div class="flex items-center gap-2 px-4 py-3 border-b" style="border-color:var(--border)">
                  ...
                  <span class="text-[11px] theme-muted">{{ post.date || '—' }}</span>
                  <span v-if="post.embedded"
                    class="ml-auto flex items-center gap-1 text-[10px] text-teal-400">
                  ...
                  <button @click="removePost(post._id)"
                    class="ml-auto text-slate-400 hover:text-rose-400 transition-colors p-1">
          ...
          <!-- Embed button -->
          <div v-if="topPosts.length" class="mt-5 flex items-center gap-3">
            <button @click="embedAll" :disabled="tpEmbedding"
              class="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-medium hover:bg-blue-500 transition-all disabled:opacity-50 shadow-sm active:scale-95">
          ...
          <div class="mt-5 p-4 rounded-xl bg-blue-500/5 border border-blue-500/15 shadow-inner">
            ...
            <div class="grid grid-cols-3 gap-4">
              ...
              <div class="text-center">
                <p class="text-lg font-display font-700 text-teal-400">{{ topPosts.length }}</p>
                <p class="text-[10px] theme-muted">{{ t('branding.totalPosts') }}</p>
              </div>
              <div class="text-center">
                <p class="text-lg font-display font-700 text-blue-400">{{ topPosts.length ? Math.round(topPosts.reduce((a,p)=>a+(p.stats?.engagementRate||0),0)/topPosts.length) : 0 }}%</p>
      ...
      <!-- ── Add Post Modal (image_60143c.png) ── -->
      <Teleport to="body">
        <Transition name="modal-fade">
          <div v-if="showAddModal"
            ...
            <div class="relative w-full max-w-lg rounded-2xl theme-surface theme-shadow overflow-hidden flex flex-col transition-all"
              style="border:1px solid var(--border); max-height:90vh">
              ...
              <div class="flex items-center justify-between px-6 py-4 border-b" style="border-color:var(--border)">
                ...
                <button @click="showAddModal = false" class="theme-muted hover:theme-text transition-colors p-1 rounded-lg hover:bg-slate-500/10">
              ...
              <div class="flex border-b bg-slate-500/5" style="border-color:var(--border)">
                <button v-for="m in [{id:'manual',icon:'✍️',label:t('branding.tpModeManual')},{id:'link',icon:'🔗',label:t('branding.tpModeLink')},{id:'doc',icon:'📄',label:t('branding.tpModeDoc')}]"
                  :key="m.id" @click="addMode = m.id"
                  class="flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-medium transition-all border-b-2 -mb-px"
              ...
              <div class="flex-1 overflow-y-auto p-6 space-y-4">
                <template v-if="addMode === 'link'">
                  <div class="flex gap-2">
                    <input v-model="linkUrl" type="url" :placeholder="t('branding.tpLinkPlaceholder')"
                      class="flex-1 px-3 py-2.5 rounded-xl theme-input theme-border text-sm theme-text focus:outline-none focus:border-blue-500/40"/>
                    <button @click="fetchFromLink" :disabled="addLoading || !linkUrl"
                      class="px-4 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-medium hover:bg-blue-500 disabled:opacity-50 transition-colors flex items-center gap-1 shadow-sm">
                ...
                <template v-if="addMode === 'doc'">
                  <div class="border-2 border-dashed border-theme rounded-xl p-8 text-center hover:border-blue-500/40 hover:bg-blue-500/5 transition-all cursor-pointer"
                    ...
                    <input ref="docInput" type="file" accept=".pdf,.docx,.doc,.png,.jpg,.jpeg,.webp" class="hidden" @change="handleDocFile"/>
                  </div>
                ...
                <template v-if="addMode === 'manual' || newPost.content">
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label class="text-[11px] theme-muted mb-1 block font-medium">{{ t('branding.tpPlatform') }}</label>
                      <select v-model="newPost.platform"
                        ...
                        <option v-for="p in ['Instagram','Facebook','LinkedIn','Twitter/X','TikTok']" :key="p" class="bg-slate-800 text-white">{{ p }}</option>
                      </select>
                    </div>
                    <div>
                  ...
                  <div>
                    <label class="text-[11px] theme-muted mb-2 block font-semibold">📊 {{ t('branding.tpStats') }}</label>
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      <div v-for="stat in ['likes','comments','shares','reach','saves']" :key="stat">
                        <label class="text-[10px] theme-muted capitalize block mb-1 font-medium">{{ t('branding.stat' + stat.charAt(0).toUpperCase() + stat.slice(1)) }}</label>
                      ...
                      <div>
                        <label class="text-[10px] theme-muted block mb-1 font-medium">{{ t('branding.statEngagement') }} %</label>
                ...
                <p v-if="addError" class="text-xs text-rose-400 bg-rose-500/10 px-3 py-2 rounded-xl border border-rose-500/20">{{ addError }}</p>
              </div>

              <div class="flex gap-3 px-6 py-4 border-t bg-slate-500/5" style="border-color:var(--border)">
...
const platformOptions = [
  ...
  { name: "X",         icon: "🕈",  labelKey: "branding.platform.x" },
