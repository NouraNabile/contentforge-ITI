<template>
  <div class="min-h-screen theme-bg flex flex-col">
    <!-- TOP NAVBAR -->
    <header
      class="theme-glass border-b sticky top-0 z-50 px-5 py-3 flex items-center justify-between"
    ...
    >
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="relative">
          ...
          >
            <div
              ...
              style="border-color: var(--border)"
            >
              <span class="font-display font-600 text-sm theme-text"
                >Notifications</span
              >
              <button
                @click="markAllRead"
            ...
            <div class="max-h-80 overflow-y-auto scrollbar-thin">
              ...
              >
                <div
                  class="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 text-sm"
                  :class="{
                    'bg-green-500/15': n.type === 'success',
                    'bg-amber-500/15':
                      n.type === 'trend' || n.type === 'warning',
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <span
                      v-if="!n.read"
                      class="w-1.5 h-1.5 bg-blue-500 rounded-full shrink-0"
                    ></span>
                  </div>
                  <p class="text-[11px] theme-sub leading-relaxed mt-0.5">
                    {{ n.message }}
                  </p>
        ...
        <!-- Avatar -->
        <div
          class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-teal-400 flex items-center justify-center text-xs font-bold text-white"
        >
    ...
    <div class="flex flex-1 overflow-hidden">
      <!-- SIDEBAR -->
      <aside
        class="w-56 border-r flex flex-col py-5 px-3 shrink-0 theme-sidebar overflow-y-auto scrollbar-thin"
        style="border-color: var(--border)"
      >
        <!-- Brand chip -->
        <div class="mb-5 px-2">
          <p
            class="text-[10px] theme-muted uppercase tracking-wider mb-2 font-medium"
          >
            Active Brand
          </p>
          <div
            class="flex items-center gap-2.5 p-2.5 rounded-xl border cursor-pointer transition-colors"
            style="
          ...
          >
            ...
            <svg
              class="w-3 h-3 theme-muted shrink-0"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
...
const stats = ref([
  { label: "Generated", value: "…", color: "text-blue-400" },
  { label: "Approved", value: "…", color: "text-green-400" },
  { label: "Scheduled", value: "…", color: "text-blue-400" },
  { label: "Published", value: "…", color: "text-teal-400" },
]);
...
async function fetchStats() {
  ...
  try {
    ...
    // تحديث badges الـ nav
    sidebarStats.value = {
      calendars: data.calendars ?? 0,
      drafts: data.drafts ?? 0,
    };
  } catch {
    // لو فشل يفضل الـ default بدون كراش
  } finally {
...
// ── Nav items — badgeKey بدل badge ستاتيك ───────────────────────────────────
const navItems = [
  {
    label: "Calendar",
    path: "/dashboard",
