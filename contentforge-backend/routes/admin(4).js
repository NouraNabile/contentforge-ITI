...
const { sendPolicyWarningEmail, sendTrialUpdateEmail, sendAdminPromotionEmail, sendPlanUpdateByAdminEmail } = require('../services/emailService')
...
router.put('/users/:id', adminOnly, async (req, res) => {
  try {
    ...
    // الـ admin مش بيتغير له plan أو planEndsAt
    ...
    // بعت الإيميل المناسب بعد الحفظ
    if (updateData.isAdmin === true && !targetUser.isAdmin) {
      // اتعمل admin جديد
      sendAdminPromotionEmail(user.email, user.name)
        .catch(err => console.error('[Email] Admin promotion error:', err.message))
    } else if (!updateData.isAdmin && updateData.plan) {
      // اتغيرت الباقة أو الـ trial
      sendPlanUpdateByAdminEmail(user.email, user.name, user.plan, user.isTrial, user.planEndsAt)
