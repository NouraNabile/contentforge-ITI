<template>
  ...
  <div v-if="isDropdownOpen" class="notifications-dropdown">
    <div class="dropdown-header">new notification</div>
    <div class="dropdown-body">
      
      <div v-if="notifications.length === 0" class="empty-notif">
        there is no un read notification
      </div>
...
import { RouterLink, RouterView, useRoute, useRouter } from 'vue-router' // ضفنا useRouter هنا
...
const router           = useRouter() // لتفعيل التوجيه البرمجي داخل الـ setup
...
// دالة فتح وقفل قائمة الإشعارات
const toggleDropdown = () => {
  isDropdownOpen.value = !isDropdownOpen.value
}

// الدالة اللي هتستدعيها لما تحبي تضيفي إشعار مستخدم جديد
// const triggerNewUserNotification = () => {
//   notifications.value.push({ text: 'New User Registered!' })
// }

// / جيب الـ  users اللي طلبوا deletion من الـ backend
    ...
    pending.forEach(u => {
      ...
      if (!exists) {
        notifications.value.push({
          userId: u._id,
          text: `${u.name} (${u.email}) requested account deletion`
        })
      }
    })
  } catch (err) {
...
// دالة الضغط على الإشعار للانتقال لصفحة المستخدمين
const goToUsersPage = (index) => {
  notifications.value.splice(index, 1) // حذف الإشعار المقروء
  isDropdownOpen.value = false         // قفل القائمة
  router.push('/admin/users')          // التوجيه للمسار
}
// ----------------------------------
...
let timer
onMounted(() => {
  fetchDeletionNotifications()  // ✅ أضيفي السطر ده
  const tick = () => {
...
/* الحاوية الأساسية تحافظ على حجم الزرار الأصلي */
...
/* زرار الجرس بتاعك بدون أي خلفيات غريبة */
.notif-btn {
  position: relative;
  background: none;
  border: none;
  padding: 6px;
  cursor: pointer;
  color: inherit; /* بياخد نفس لون الـ Navbar عندك */
  display: flex;
  align-items: center;
  justify-content: center;
}

/* الدائرة الحمراء بتتحط بدقة فوق الجرس */
.notif-badge {
  position: absolute;
  top: 2px;
  right: 2px;
...
/* القائمة المنسدلة (بتطير فوق التصميم ومبتزقش حاجة) */
.notifications-dropdown {
  position: absolute;
  top: 100%; /* بتظهر تحت الزرار عل طول */
  right: 0;
  width: 250px;
  background: #ffffff;
  ...
  z-index: 9999; /* عشان تظهر فوق أي كلام أو جداول تحتها */
  margin-top: 8px;
  overflow: hidden;
}
