      ...
      <div class="flex h-[440px]">
        ...
        <div class="w-52 border-r border-white/5 bg-forge-900 flex flex-col py-4 px-3 shrink-0">
          ...
          <div class="flex items-center gap-2 px-2 py-2 rounded-lg bg-blue-600/10 border border-blue-500/20 mb-6">
            <div class="w-6 h-6 rounded-md bg-gradient-to-br from-blue-500 to-teal-400 flex items-center justify-center text-xs font-bold text-white">A</div>
            <div class="flex-1 min-w-0">
              <p class="text-xs font-medium text-white truncate">Araby Coffee</p>
              <p class="text-[10px] text-slate-500">Egyptian Market</p>
            </div>
          ...
          <nav class="space-y-0.5 flex-1">
            <div v-for="(item, i) in sidebarItems" :key="item.label"
              ...
              {{ item.label }}
              <span v-if="item.badge" class="ml-auto text-[9px] px-1.5 py-0.5 rounded-full bg-blue-600/20 text-blue-400">
                {{ item.badge }}
              </span>
            </div>
          </nav>
          ...
          <div class="mt-4 p-2.5 rounded-lg bg-forge-950/60 border-subtle">
            <p class="text-[10px] text-slate-500 mb-2">This Month</p>
            <div class="space-y-1.5">
              <div class="flex justify-between text-[11px]">
                <span class="text-slate-400">Posts Generated</span>
        ...
        <div class="flex-1 overflow-hidden flex flex-col">
          ...
          <div class="flex items-center justify-between px-5 py-3.5 border-b border-white/5">
            <div>
              <h2 class="text-sm font-display font-600 text-white">Content Calendar</h2>
              <p class="text-[11px] text-slate-500">May 26 – June 8, 2026</p>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-[10px] px-2 py-1 rounded-md bg-amber-500/10 text-amber-400 border border-amber-500/20">
                ✦ Ramadan Campaign Active
              </span>
              <button class="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-[11px] font-medium hover:bg-blue-500 transition-colors">
                + Generate Plan
              </button>
            </div>
          </div>
          ...
          <div class="flex-1 overflow-hidden p-4">
            ...
            <div class="grid grid-cols-7 gap-1.5 mb-2">
              <div v-for="day in weekDays" :key="day"
                class="text-center text-[10px] text-slate-500 py-1">
                {{ day }}
              </div>
            </div>
            <!-- Calendar cells -->
            <div class="grid grid-cols-7 gap-1.5">
              <div v-for="cell in calendarCells" :key="cell.id"
                ...
                <div v-if="cell.post" class="flex-1">
                  ...
                  <p class="text-[9px] leading-tight" :class="cell.textClass">{{ cell.post }}</p>
                </div>
                <div v-if="cell.status"
                  class="mt-auto text-[8px] font-medium"
        ...
        <div class="w-52 border-l border-white/5 bg-forge-800/30 flex flex-col py-4 px-3 shrink-0">
          <p class="text-[10px] text-slate-500 font-medium uppercase tracking-wider mb-3">Edit Post</p>
          <!-- Selected post editor mockup -->
          <div class="bg-forge-950/60 rounded-lg p-3 border-subtle mb-3">
            <div class="flex items-center gap-1.5 mb-2">
              <span class="text-[9px] px-1.5 py-0.5 rounded-full bg-pink-500/15 text-pink-400">Instagram</span>
              <span class="text-[9px] text-slate-500">Arabic (EGY)</span>
            </div>
          ...
          <!-- Status selector -->
          <p class="text-[10px] text-slate-500 mb-1.5">Status</p>
          ...
          <div class="mt-auto p-2.5 rounded-lg bg-forge-950/60 border border-amber-500/15">
            <p class="text-[9px] text-amber-400 font-medium mb-1">A/B Critic Agent</p>
            <p class="text-[9px] text-slate-500 leading-relaxed">
              Version B suggests stronger hook — leads with the offer.
            </p>
            <button class="mt-2 w-full py-1 rounded-md bg-amber-500/10 text-amber-400 text-[9px] hover:bg-amber-500/20 transition-colors">
              View Version B
            </button>
          </div>
        </div>
...
<script setup>
const sidebarItems = [
  { label: 'Calendar', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z', badge: '12' },
  { label: 'Brand Vault', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
  { label: 'Image Gen', icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z' },
  ...
  { label: 'Publish', icon: 'M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z' },
...
const statuses = [
  { label: 'Draft', dot: 'bg-slate-500', active: false },
  { label: 'Pending Review', dot: 'bg-amber-400', active: true },
  { label: 'Approved', dot: 'bg-green-400', active: false },
  { label: 'Scheduled', dot: 'bg-blue-400', active: false },
  { label: 'Published', dot: 'bg-teal-400', active: false },
]
</script>
